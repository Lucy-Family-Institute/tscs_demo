...
Intuition here is that the transformed LDV and error are correlated. The group mean of the LDV contains all realizations of $Y$ minus the final observation of the unit.  Any $Y_t$ is correlated with the appropriate error $u_t$, which is introduced into the error term by the group difference.

The FE estimator is consistent, but the asymptotics are in $T$ not $N$ - so in short panels the bias can be very large.
