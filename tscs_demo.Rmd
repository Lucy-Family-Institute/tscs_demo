...
$Y_{it}=\beta_1X_{it}+\beta_2\overline{X_i}+\epsilon_{it}$
...
$Y_{it}=\beta_1(X_{it}-\overline{X_i})+\beta_2\overline{X_i}+\epsilon_{it}$
...
Again, we've successfully purged the coefficients on the individual realizations of $X$.  Our coefficient on $\overline{X}$ has changed.  The REWB estimate was `r coef(rebw)[3]` which we can see here is actually the sum of the between and within coefficients from the Mundlak estimator `r coef(mundlak)[3]` and `r coef(mundlak)[2]`
