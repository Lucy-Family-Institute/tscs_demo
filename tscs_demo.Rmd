...
This does not mean OLS is the absolute best model in any circumstance 
...
The formula for coefficients of multivariate regression are more complicated, but one representation:

$$\hat\beta_1 = \frac{\sum_{i=1}^{n}\hat{r_{i1}}y_i}{\sum_{i=1}^{n}(\hat{r_{i1}})^2} $$
Where $\hat{r_{i1}}$ is the residual from the auxiliary regression of $X1$ on $X2$
...
1) $E[\hat{r}] = 0 $ by regression assumptions
2) $Cov(x, y) = \sum_{i=1}^{n}(x_i - \overline{x})(y_i - \overline{y}) = \sum_{i=1}^{n}(x_i - \overline{x})y_i $
