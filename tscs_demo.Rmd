...
OLS estimates the coefficients $\beta$ that minimize $RSS$.  These can be shown to be:
...
This does not mean OLS is the absolute best model in any circumstances. 

If the standard errors are not white noise, then non-linear unbiased estimators may offer improvement, such as trimmed least squares

If the errors are white-noise, variance may be reduced by using a biased estimator, such as ridge regression
